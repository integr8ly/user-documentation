...
All of the applications and services provided by {PRODUCT_SHORT} are protected by your identity provider.
...
Typically, {cluster-developer}s start by xref:creating-using-solution-patterns_getting-started[].
...
The {webapp} is a web-based UI with single sign-on access to all the applications and services available in your {PRODUCT_SHORT} cluster:
...
The {webapp} also provides xref:introduction-to-solution-patterns[Solution Patterns] that provide step-by-step instructions and best practices for common connectivity and application development tasks that create new {solution-name}.
...
* To learn more about using Solution Patterns, see xref:creating-using-solution-patterns_getting-started[].
...
[id="creating-using-solution-patterns_{context}"]

= Creating {solution-name} using Solution Patterns
...
. Perform the *5 - Adding Data Sync to your application with GraphQL* Solution Pattern to learn about building a Node.js application using a GraphQL API. This Solution Pattern also shows how to integrate applications with Red Hat Single-Sign On and AMQ Online.
