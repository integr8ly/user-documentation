...
{PRODUCT} provides a simplified cloud-native experience and includes:

* Solution Explorer: A web-based UI with single sign-on access to all the applications.
* Solution Patterns: Pre-built templates that shorten development time and encourage best practices.
* Recommended configurations: Installed infrastructure components are preconfigured for optimal performance.
* Integrated technology: Unified development tools and services.
...
The {webapp} includes links to the applications and services available in your {PRODUCT_SHORT} cluster.


{PRODUCT_SHORT} provides access to the following application consoles:

* *OpenShift*, the OpenShift cluster console.
+
NOTE: From the OpenShift Console, you can navigate to the Solution Explorer by clicking the Application Launcher {app-launcher} and choosing *Solution Explorer*.
...
The {webapp} also provides xref:what-are-solution-patterns[Solution Patterns] that guide you through the process of creating various integrated solutions using these applications.
...
Solution Patterns are tutorials available in your {PRODUCT_SHORT} cluster. The Solution Patterns provided in {webapp} guide you through the following scenarios:
...
Before {cluster-developer}s start working in the cluster, complete this section to configure your cluster.
...
. Log in to the {webapp}. The URL for the {webapp} is included with the *Welcome email*.
...
. Complete the xref:configuring-github-oauth-for-codeready-workspaces_getting-started[] section so that developers can use {code-ready-short} with GitHub.
...
. Give 3scale administrative privileges to any {cluster-developer}s who need to create products in 3scale. This is described in the link:https://access.redhat.com/documentation/en-us/red_hat_3scale_api_management/2.8/html/admin_portal_guide/inviting-users-managing-rights#give_new_users_rights[3scale Admin Portal Guide].
+
...
* See the link:{BASE_URL}/administration_guide_for_red_hat_managed_integration_2/[{PRODUCT_SHORT} Administration Guide] for more information on administering your cluster.
...
= Preparing to develop {solution-name}
...
+
NOTE: See link:{BASE_URL}/developer_guide_for_red_hat_managed_integration_2/index#adding-solution-patterns[Adding Solution Patterns] for information about how to subscribe your cluster to a Git repository that contains more Solution Patterns.
...
* See the link:{BASE_URL}/developer_guide_for_red_hat_managed_integration_2[Developer Guide] for more information about what {cluster-developer}s can do in a cluster.
