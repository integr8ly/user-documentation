...
NOTE: Red Hat recommends that you define users of the solution you are developing in the `{customer-realm-name}` realm. For example, if you use {PRODUCT} to develop an solution named `my-app`, define all users of `my-app` in the `{customer-realm-name}` realm only.
