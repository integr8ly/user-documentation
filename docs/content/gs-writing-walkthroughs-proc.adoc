...
= Writing {PRODUCT} {walkthrough-name}s


== About writing {walkthrough-name}s
...
----
= Demonstrating a custom {walkthrough-name} <1>

A short description for the {walkthrough-name}. <2>
...
[source,bash]
----
git clone https://github.com/integr8ly/{walkthrough-name}-template.git
----
...
└── walkthroughs<1>
    ...
    │   ├── walkthrough.adoc <4>
    │   └── walkthrough.json <5>
    ├── Another {walkthrough-name} <2>
    └── Third {walkthrough-name} <2>
----

<1> This is a required directory. All {walkthrough-name}s are defined under `walkthroughs`.

<2> The directory name of a {walkthrough-name}. This is also displayed in the URL of the web app. 

<3> All images for the {walkthrough-name}. 

<4> *walkthrough.adoc* is where the content of the {walkthrough-name} is written in asciidoc format.
<5> *walkthrough.json* is the manifest file of the {walkthrough-name} where you can define extra dependencies.
...
----
NOTE: If you want to provision services as part of the {walkthrough-name}, this process is described in <<Adding custom services to a {walkthrough-name}>>
...
* Custom attributes:
** NodeJS Frontend App Route (provisioned from {walkthrough-name}.json): `\{route-frontend-host}`
