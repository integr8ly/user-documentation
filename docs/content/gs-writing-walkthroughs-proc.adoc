...
<2> This is a an optional file that defines metadata for the set of walkthroughs in the repo. 
...
By default, the walkthroughs from a repo are displayed in Solution Explorer with a title corresponding to your repo name. However, you can add a `walkthroughs-config.json` file with the following contents if, for example, you want to title the group `My Solution Patterns`:
